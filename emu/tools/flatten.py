#!/usr/bin/env python3
"""Generate purv.c: the whole RISC-V engine as one hidden implementation file.

atoom ships header-only (~45 self-guarded headers of static-inline code). A
normal C library hides all of that. This emits a single purv.c that:

  - #includes the slim public header purv.h (opaque state + public prototypes);
...
extensions (A, B/Zb*) and the RVE_E_HOOK instrumentation are stripped out.
  ...
  usage: flatten.py <atoom/include> <out_dir>      (writes <out_dir>/purv.c)
"""
import os, re, sys
...
# tagged struct matching `typedef struct RiscvEmulatorState ...;` in purv.h.
